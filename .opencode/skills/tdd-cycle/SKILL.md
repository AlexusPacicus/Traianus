...
4. Verify: confirm `.github/workflows/ci.yml` covers any new/modified test paths and dependencies (AGENTS.md §1.6).

Rules:
- Process one phase at a time; do not advance until the executor reports success.
- Empirical validation: `pytest tests/ -q` and `python3 tools/audit/audit_harness.py` (C1 GUARD).
